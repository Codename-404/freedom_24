...
export default function HelpCard({ info }) {
  //   const { details, phone, name, lat, lng } = info;
  const details =
  ...
  return (
    <div className="w-full h-44 flex flex-row gap-2 p-2 bg-neutral-800 rounded-xl">
      <img
      ...
      <div className="w-full h-full flex flex-col gap-2 justify-between">
        <h2 className="text-xs tracking-wider">
          {details.length > 250 ? `${details.substring(0, 251)}...` : details}
        </h2>
        ...
        <div className="w-full flex text-sm justify-between">
          ...
          <button
            ...
            className="bg-red-500 rounded-md w-40 pt-1"
          >
            View location
          </button>
        </div>
